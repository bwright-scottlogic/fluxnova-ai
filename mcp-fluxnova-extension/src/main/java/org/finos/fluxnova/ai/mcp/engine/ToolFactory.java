...
import org.finos.fluxnova.ai.mcp.model.ToolParameter;
import org.finos.fluxnova.ai.mcp.registry.ToolConfig;
 ...
 * Bridges the gap between BPMN process definitions and MCP tool registry.
 */
public class ToolFactory {
    ...
    public void createAndRegister(ToolDefinition definition) {
        try {
            ...
            LOG.info("Registered MCP tool '{}' for process '{}'",
                    definition.toolName(), definition.processId());

        } catch (Exception e) {
            LOG.error("Failed to create and register tool '{}' for process '{}'",
